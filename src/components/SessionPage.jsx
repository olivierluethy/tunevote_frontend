...
const SessionPage = () => {
  ...
  const handlePasteLink = async () => {
    try {
      ...
      if (text) {
        setSearchQuery(text); // ⬅️ ggf. an deinen State anpassen
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      "
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6">
          ...
          {/* Hinweis + Paste Button */}
          <div className="flex items-center justify-between gap-3 mb-3 bg-indigo-50 border border-indigo-200 rounded-lg p-3">
            <p className="text-sm text-indigo-700">
              🎵 Can't find the song? Just add a{" "}
              <span className="font-medium">YouTube link</span>.
            </p>

            <button
              onClick={handlePasteLink}
              className="px-3 py-1.5 text-sm rounded-full bg-indigo-500 text-white hover:bg-indigo-600 transition shadow"
            >
              📋 Paste Link
            </button>
          </div>
